...
async def song_command(client, message):
    if len(message.command) < 2:
        await message.reply_text(
            f"{e.NOTE} <b>Download a song</b>\n"
            "<code>/song &lt;name or YouTube/Spotify/Resso/SoundCloud link&gt;</code>\n"
            "<i>I'll fetch the audio as an mp3 and send it here.</i>", parse_mode=_HTML)
        return
    ...
    try:
        ...
        await log_download(client, user=message.from_user, url=url,
                           media_type="audio", ok=False, started=t0,
                           error=f"{type(exc).__name__}: {exc}")
        return

    duration = int(info.get("duration") or 0)
    performer = info.get("uploader") or info.get("channel") or "Unknown"
    # yt-dlp's `thumbnail` is a REMOTE URL — pyrofork opens thumb as a
    # LOCAL path and FileNotFoundError's the whole upload. Drop it;
    # Telegram renders fine without. (Same fix linksniffer already has.)
    ...
    try:
        ...
        await client.send_audio(
            ...
            audio=path,
            caption=f"{e.NOTE} <b>{html.escape(str(title))}</b>",
            parse_mode=_HTML,
            title=title,
            performer=performer,
            duration=duration,
        ...
        await status.delete()
        await log_download(client, user=message.from_user, url=url,
                           media_type="audio", ok=True, started=t0,
