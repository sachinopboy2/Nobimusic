...
async def unban_command(client, message):
    if message.chat.type not in (ChatType.GROUP, ChatType.SUPERGROUP):
        await message.reply_text(
            ...
            "<i>/unban works inside a group.</i>", parse_mode=_HTML)
        return

    if not await _is_admin(client, message.chat.id, message.from_user.id):
        await message.reply_text(
            "🔒 <b>Admins only</b>\n"
            "<i>Only group admins can /unban.</i>", parse_mode=_HTML)
        return
    ...
    if target is None:
        await message.reply_text(
            f"{e.SHIELD} <b>Unban — how to use</b>\n"
            "• <i>Reply to a message with</i> <code>/unban [reason]</code>\n"
            "• <code>/unban &lt;user_id&gt; [reason]</code>\n"
            "• <code>/unban @username [reason]</code>", parse_mode=_HTML)
        return
