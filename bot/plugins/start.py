...
def _start_caption(user) -> str:
    ...
    return (
        f"{e.STAR}  <b>𝑾𝑬𝑳𝑪𝑶𝑴𝑬 𝑻𝑶 {BOT_NAME.upper()}</b> {e.MUSIC_NEW}\n\n"
        f"Hey {mention}!\n"
        f"I'm {BOT_NAME}, your premium music companion for Telegram Voice Chats.\n\n"
        f"{e.BOLT} Fast {e.DOT} {e.MUSIC} High Quality Audio\n"
        f"{e.BRAIN} Smart Queue {e.DOT} {e.FIRE} Powerful Playback\n"
        f"{e.PEOPLE} Group Friendly {e.DOT} {e.HEAD} 24/7 Music\n\n"
        f"{e.SEPARATOR}\n\n"
        f"{e.USER} <b>Your Profile</b>\n"
        f"{e.HEART_ICON} User: {mention}\n"
        f"{e.BOT_TAG} ID: <code>{user.id}</code>\n\n"
...
async def _start_keyboard(client, styled=True):
    ...
    top = []
    if updates_url:
        top.append(add("Updates", url=updates_url,
                       emoji_id=e.MEGA_ID, style=ButtonStyle.PRIMARY))
    if support_url:
        top.append(add("Support", url=support_url,
                       emoji_id=e.CHAT_ID, style=ButtonStyle.SUCCESS))
    if top:
        rows.append(top)

    if owner_url:
        rows.append([add("Owner", url=owner_url,
                         emoji_id=e.CROWN_ID, style=ButtonStyle.PRIMARY)])

    add_url = await _resolve_add_url(client)
    if add_url:
        rows.append([add("Add Me to Your Group", url=add_url,
                         emoji_id=e.PLUS_ID, style=ButtonStyle.SUCCESS)])

    rows.append([add("Help & Commands", callback_data="help:0:home",
                     emoji_id=e.BOOK_ID, style=ButtonStyle.PRIMARY)])
